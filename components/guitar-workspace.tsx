...
function stringCenterPercent(string: number) {
  ...
  // Player/tab view: string 1 high-e is on top, string 6 low-E is on bottom.
  const topCenter = 16.27;
  const stringGap = 13.49;
  return topCenter + (string - 1) * stringGap;
}
...
function ChordDiagram({ chord, markers }: { chord: string; markers: NoteMarker[] }) {
  ...
  return (
    ...
    >
      ...
      <section className={`instrument-stage ${isPlaying ? "is-playing" : ""}`} aria-label="Interactive guitar stage">
        ...
        {hasStarted ? <ChordDiagram chord={activeChord} markers={markers} /> : null}
      </section>
