...
/** Keep whole-photo contact dots small enough to distinguish adjacent strings. */
export function photoMarkerDiameter(guitar: DreamGuitar, width: number) {
  const safeWidth = Math.max(1, width);
  const axes = guitar.photoStrings;
  const gap = axes ? (axes.nut[1] - axes.nut[0]) * safeWidth / 5 : safeWidth * .8 / guitar.photoScale / 5;
  return Math.min(3.2, gap * .76);
}
