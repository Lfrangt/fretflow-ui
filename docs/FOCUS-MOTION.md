...
The ordinary joined scene now includes the selected photograph's own headstock, cropped at its calibrated nut and rendered at the body photograph's scale. For the calibrated Relic, the ordinary neck follows the measured nut-to-body taper, with all six strings and markers projected onto that same profile. There is no added flared connector. A longer teaching neck gives the body more room on the right; 80px desktop chord cards return another 10px to the stage. The headstock fades with the body during Focus and returns on exit. Whole-photo phone views keep the original full guitar.
...
The ordinary profile's four string endpoints interpolate into the fixed rectangular Focus grid with the same 1.2-second progress. Wood outline, frets, nut, strings and markers consume that shared progress; profile CSS variables are applied directly, without a second nested Motion animation. Native rendering interpolates its presentation rectangle, scale and profile together. Steady Focus browsing still changes only position. Ordinary markers fit the narrowest measured string spacing; Focus retains its original marker size. Other photographs without measured string axes keep their existing fallback geometry.
