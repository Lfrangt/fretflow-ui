...
- 上方动画指板显示音程，是主要演奏参考。完整练习指板从第 1 品保留至第 21 品；品格按真实十二平均律比例逐渐缩窄，手机最窄的第 21 格仍保留约 30px，屏宽不足时横向裁切并跟随当前把位，普通与 Focus 模式一致。
...
- 弹层的 Escape 只关闭弹层；页面上的 Escape 退出 Focus。桌面练习围绕当前把位放大，切换把位时平滑移动镜头与指位。
